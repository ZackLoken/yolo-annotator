"""Tests for yololabeler.predictions.importers, the three prediction converters."""
...
class TestBurDetectJson:
    def test_converts_pixels_to_canonical(self, tmp_path, folder):
        ...
        result = import_predictions(src, folder, "bur_detect_json", "nathan_v15", 0, "zack")
    ...
    def test_counts_rotated_images(self, tmp_path, folder):
        src = tmp_path / "src"
        src.mkdir()
        (src / "b.json").write_text(json.dumps({"boxes": [], "scores": []}), encoding="utf-8")
        result = import_predictions(src, folder, "bur_detect_json", "m", 0, "z")
    ...
    def test_skips_files_without_image(self, tmp_path, folder):
        src = tmp_path / "src"
        src.mkdir()
        (src / "zzz.json").write_text(json.dumps({"boxes": [], "scores": []}), encoding="utf-8")
        result = import_predictions(src, folder, "bur_detect_json", "m", 0, "z")
...
class TestUltralyticsTxt:
    def test_moves_confidence_to_column_two(self, tmp_path, folder):
        src = tmp_path / "src"
        src.mkdir()
        (src / "a.txt").write_text("0 0.5 0.5 0.2 0.2 0.9\n2 0 0 0.5 0 0.5 0.5 0.7\n",
                                   encoding="utf-8")
        result = import_predictions(src, folder, "ultralytics_txt", "m", None, "z")
...
class TestYololabeler:
    def test_copies_after_validation(self, tmp_path, folder):
        ...
        (src / "detect" / "a.txt").write_text("0 0.9 0.5 0.5 0.2 0.2\nbad\n", encoding="utf-8")
