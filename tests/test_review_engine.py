    ...
    def test_save_and_load(self, engine):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            engine.state._review_state = {
                "image": {"a.jpg": {"img_status": "completed"}}
            }
            engine.save_review_state()
            engine.state._review_state = {}
            engine.load_review_state()
            assert (
                engine.state._review_state["image"]["a.jpg"]["img_status"]
                == "completed"
            )
...
def pred(pid, x1, y1, x2, y2, cid=0, conf=0.9):
    ...
    return Prediction(
        pid, "box", ((x1, y1), (x2, y2)), cid, conf, int(pid.split(":")[1])
    )


@pytest.fixture
def scene():
    """One matched box, one model miss, one false positive, one low-confidence
    pred.
    """
...
class TestQueue:
    ...
    def test_queue_follows_the_nearest_neighbour_path_from_the_top_left(
        self, scene
    ):
        doc, preds = scene
        queue = build_queue(
        ...
        # tp centre (62, 62) is nearest the corner; fn (350, 350) is nearer to
        # it than fp (530, 50).
    ...
    def test_status_filter_does_not_reorder_the_path(self, scene):
        ...
        queue = build_queue(
            doc, preds, matches, verdicts, filter_status="not_reviewed"
        )
    ...
    def test_threshold_change_reveals_it(self, scene):
        doc, preds = scene
        queue = build_queue(
            doc, preds, match_document(doc, preds, 0.6, 0.25), {}
        )
    ...
    def test_filters(self, scene):
        ...
        assert [
            q.kind
            for q in build_queue(
                doc, preds, matches, verdicts, filter_type="fn"
            )
        ] == ["fn"]
        assert [
            q.key
            for q in build_queue(
                doc, preds, matches, verdicts, filter_status="reviewed"
            )
        ] == ["h:1"]
        assert (
            len(
    ...
    def test_polygon_prediction_matches_polygon_annotation(self):
        ...
        doc.add(
            new_annotation(
                "polygon", ((0, 0), (50, 0), (50, 50), (0, 50)), 1, "z"
            )
        )
        p = Prediction(
            "h:0", "polygon", ((1, 1), (50, 0), (50, 50), (0, 50)), 1, 0.8, 0
        )
...
class TestSpatialOrder:
    def test_a_cluster_is_visited_before_moving_on(self):
        far = [
            pred("h:0", 900, 900, 910, 910),
            pred("h:1", 0, 0, 10, 10),
            pred("h:2", 600, 20, 610, 30),
            pred("h:3", 30, 0, 40, 10),
            pred("h:4", 15, 25, 25, 35),
        ]
        items = [QueueItem("fp", p, None, None) for p in far]
        assert [q.key for q in spatial_order(items)] == [
            "h:1",
            "h:4",
            "h:3",
            "h:2",
...
class TestShapeStatuses:
    ...
    def test_a_match_shares_its_status_with_its_annotation(self, scene):
        ...
        assert shape_statuses(doc, preds, matches, verdicts) == {
            "h:0": "accepted",
            doc.annotations[0].id: "accepted",
            "h:1": "rejected",
            doc.annotations[1].id: "not_reviewed",
        }

    def test_low_confidence_prediction_has_no_status(self, scene):
        doc, preds = scene
        statuses = shape_statuses(
            doc, preds, match_document(doc, preds, 0.6, 0.5), {}
        )
...
class TestActions:
    def test_accept_fp_inserts_annotation_with_provenance(self, scene):
        doc, preds = scene
        item = of_kind(
            build_queue(doc, preds, match_document(doc, preds, 0.6, 0.5), {}),
            "fp",
        )
        ...
        assert (
            created.confidence == pytest.approx(0.9)
            and created.author == "ren"
        )
        assert (
            created.points == ((500.0, 20.0), (560.0, 80.0))
            and created.class_id == 0
        )

    def test_accept_clamps_an_edge_of_frame_prediction_to_the_image(
        self, scene
    ):
        doc, preds = scene
    ...
    def test_accept_tp_and_fn_change_nothing(self, scene):
        ...
        assert apply_accept(doc, of_kind(queue, "fn"), "ren") == (
            "accepted",
            None,
        )
        assert apply_accept(doc, of_kind(queue, "tp"), "ren") == (
            "accepted",
            None,
        )
...
class TestVerdicts:
    ...
    def test_explicit_conf_threshold_overrides_imported_min(
    ...
    def test_reimport_resets_a_typed_threshold_to_the_new_min(
        self, engine, tmp_path
    ):
        engine.state.image_folder = str(tmp_path)
        engine.state.state_dir = str(tmp_path / "state")
        ...
        write_manifest(
            str(preds_dir),
            {"min_conf": 0.4, "imported_at": "2026-09-15T08:00:00"},
        )
        engine.conf_threshold = 0.6
        write_manifest(
            str(preds_dir),
            {"min_conf": 0.35, "imported_at": "2026-09-16T08:00:00"},
        )
        assert engine.conf_threshold == pytest.approx(0.35)

    def test_unstamped_saved_threshold_yields_to_imported_min(
        self, engine, tmp_path
    ):
        engine.state.image_folder = str(tmp_path)
        engine.state.state_dir = str(tmp_path / "state")
...
class TestImgStatus:
    def test_freshly_created_entry_is_not_started(self, engine):
        ...
        assert (
            engine.state._review_state["image"]["img_001.jpg"]["img_status"]
            == "not_started"
        )
    ...
    def test_filtering_has_no_bearing_on_the_computed_status(
        self, engine, scene
    ):
        doc, preds = scene
        engine.state.document = doc
        ...
        # A "reviewed" filter would make this look 1-of-1 complete;
        # update_img_status must not use it.
        filtered = build_queue(
            doc,
            preds,
...
class TestMigrateCentreEntries:
    def test_moves_matching_entries_and_drops_the_rest(self, engine, tmp_path):
                ...
                "img_001.jpg": {
                    ...
                    "detections": [
                        ...
                        },
                        {
                            ...
                            "reviewed_by": "zack",
                            "class_id": 0,
                            "pred_bbox_norm": [0.1, 0.1, 0.05, 0.05],
                            "gt_bbox_norm": None,
                            "iou": None,
                            "conf": 0.7,
                        },
        ...
        assert (
            "detections"
            not in engine.state._review_state["image"]["img_001.jpg"]
        )
...
def flag_item(engine, item, comment, user):
    engine.save_flag(
        "img_001.jpg", item.key, item.kind, item.class_id, comment, user
    )
...
class TestFlags:
    def test_save_opens_a_flag_without_a_verdict(self, flag_engine, scene):
        ...
        assert (
            ...
            and flag["edited_by"] is None
        )
        assert flag_engine.verdicts("img_001.jpg") == {}
        assert flag_engine.open_flag_keys("img_001.jpg") == {item.key}

    def test_editing_keeps_the_flagger_and_records_the_editor(
        self, flag_engine, scene
    ):
        doc, preds = scene
        item = of_kind(
            build_queue(doc, preds, match_document(doc, preds, 0.6, 0.5), {}),
    ...
    def test_saving_an_unchanged_comment_records_no_edit(
        self, flag_engine, scene
    ):
        doc, preds = scene
        item = of_kind(
        ...
        assert (
            flag_engine.open_flag("img_001.jpg", item.key)["edited_by"] is None
        )

    def test_resolve_keeps_the_record_and_a_new_flag_starts_a_new_entry(
        self, flag_engine, scene
    ):
        doc, preds = scene
        item = of_kind(
        ...
        assert (
            flag_engine.open_flag("img_001.jpg", item.key)["comment"]
            == "again"
        )
    ...
    def test_resolve_records_a_note(self, flag_engine, scene):
        ...
        flag_item(flag_engine, item, "", "ren")
        flag_engine.resolve_flag(
            "img_001.jpg", item.key, "ren", note="rejected"
        )
        assert (
            flag_engine.flags("img_001.jpg")[item.key][0]["resolved_note"]
            == "rejected"
        )

    def test_flags_persist_and_has_open_flags_reads_them(
        self, flag_engine, scene
    ):
        doc, preds = scene
        item = of_kind(
    ...
    def test_carry_flags_moves_the_history_to_the_new_key(
        self, flag_engine, scene
    ):
        doc, preds = scene
        item = of_kind(
    ...
    def test_flag_key_prefers_an_annotation_flagged_before_predictions(
        self, flag_engine, scene
    ):
        doc, preds = scene
        tp = of_kind(
        ...
        flag_engine.save_flag(
            "img_001.jpg", tp.annotation.id, None, 0, "", "ren"
        )
    ...
    def test_flagged_status_filter_keeps_only_open_flagged_items(self, scene):
        ...
        matches = match_document(doc, preds, 0.6, 0.5)
        queue = build_queue(
            doc, preds, matches, {}, filter_status="flagged", flagged={"h:1"}
        )
        assert [q.key for q in queue] == ["h:1"]
        by_annotation = build_queue(
            doc,
            preds,
            matches,
            {},
