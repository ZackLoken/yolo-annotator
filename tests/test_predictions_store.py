"""Tests for yololabeler.predictions.store, prediction ids, loading and
manifest.
"""
...
class TestIds:
    def test_file_hash_is_sha1_prefix(self, tmp_path):
        ...
        assert (
            file_hash(p)
            == hashlib.sha1(b"0 0.9 0.5 0.5 0.2 0.2\n").hexdigest()[:12]
        )
...
class TestManifest:
    ...
    def test_corrupt_manifest_is_quarantined_and_read_as_absent(
        self, tmp_path
    ):
