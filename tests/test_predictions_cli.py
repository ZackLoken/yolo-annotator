"""Tests for yololabeler.predictions.cli, the headless yololabeler-import entry point."""
...
@pytest.fixture
def tree(tmp_path):
    """Source tree and image tree pairing on one mission, with one unmatched folder each."""
    ...
    for rel in ("Farm/North/M1", "Farm/South/M9"):
        ...
        (src / rel / "a.json").write_text(
            json.dumps({"boxes": [[10, 20, 30, 60]], "scores": [0.875]}), encoding="utf-8")
    return src, img
...
class TestHeadless:
    def test_no_tk_in_the_import_chain(self):
        """A fresh interpreter importing the CLI must load no Tk or CustomTkinter."""
...
class TestRootOverlap:
    def test_identical_roots_rejected(self, tree):
        ...
        with pytest.raises(SystemExit) as exc:
            run([img, img, "--format", "bur_detect_json", "--model", "m", "--class-id", 0])
    ...
    def test_source_nested_in_images_rejected(self, tree):
        ...
        nested.mkdir()
        with pytest.raises(SystemExit) as exc:
            run([nested, img, "--format", "bur_detect_json", "--model", "m",
                 "--class-id", 0])
        assert exc.value.code == 2
    ...
    def test_images_nested_in_source_rejected(self, tmp_path):
        ...
        with pytest.raises(SystemExit) as exc:
            run([src, nested, "--format", "bur_detect_json", "--model", "m",
                 "--class-id", 0])
        assert exc.value.code == 2

    def test_sibling_roots_accepted(self, tree):
        src, img = tree
        assert run([src, img, "--format", "bur_detect_json", "--model", "m",
                    "--class-id", 0]) == 0

    def test_missing_root_rejected(self, tmp_path, tree):
        _src, img = tree
        with pytest.raises(SystemExit) as exc:
            run([tmp_path / "nope", img, "--format", "bur_detect_json", "--model", "m",
                 "--class-id", 0])
        assert exc.value.code == 2
...
class TestDryRun:
    def test_is_the_default_and_writes_nothing(self, tree, capsys):
        ...
        assert run([src, img, "--format", "bur_detect_json", "--model", "m",
                    "--class-id", 0]) == 0
        assert files_under(src) == before_src and files_under(img) == before_img
        assert "Dry run: nothing written" in capsys.readouterr().out

    def test_prints_pairs_and_both_unmatched_lists(self, tree, capsys):
        src, img = tree
        run([src, img, "--format", "bur_detect_json", "--model", "m", "--class-id", 0])
        out = capsys.readouterr().out
        assert "Matched folder pairs (1):" in out
        assert "Farm/North/M1" in out and "Farm/North/M0" in out and "Farm/South/M9" in out
    ...
    def test_prints_stem_collisions(self, tmp_path, capsys):
        ...
        run([src, img, "--format", "bur_detect_json", "--model", "m", "--class-id", 0])
        out = capsys.readouterr().out
        assert "Source stems found under more than one source folder (1):" in out
...
class TestWrite:
    def test_writes_matched_pairs_only(self, tree, capsys):
        src, img = tree
        assert run([src, img, "--format", "bur_detect_json", "--model", "nathan_v15",
                    "--class-id", 0, "--user", "zack", "--write"]) == 0
        assert (img / "Farm/North/M1" / "predictions" / "detect" / "a.txt").read_text(
