"""Tests for yololabeler.predictions.batch, the tree-to-tree prediction
importer.
"""
...
# Farm/Field/Mission-shaped tree: two fields, two missions each, one mission
# per side that the other side has no folder for.
...
def make_json(path, stem, boxes=((10, 20, 30, 60),), scores=(0.875,)):
    ...
    (path / f"{stem}.json").write_text(
        json.dumps(
            {"boxes": [list(b) for b in boxes], "scores": list(scores)}
        ),
        encoding="utf-8",
    )
...
def stem_for(rel_path):
    """A stem unique to one mission, so a mispairing shows up as a missing
    file.
    """
...
@pytest.fixture
def tree(tmp_path):
    """Source tree and image tree that pair on MISSIONS and differ on one
    folder each.
    """
...
class TestPlanPairs:
    def test_pairs_by_relative_path_through_a_multi_level_tree(self, tree):
        ...
        assert [p.rel_path.replace("\\", "/") for p in plan.pairs] == sorted(
            MISSIONS
        )
        for pair in plan.pairs:
            assert pair.source_files == 1 and pair.images == 1

    def test_same_leaf_name_under_a_different_parent_is_not_a_match(
        self, tmp_path
    ):
        src, img = tmp_path / "preds", tmp_path / "images"
        ...
        make_json(
            src / "Farm/South/M1", "a"
        )  # same leaf name, different branch
        plan = plan_pairs(src, img, "bur_detect_json")
        assert plan.pairs == []
        assert [p.replace("\\", "/") for p in plan.unmatched_images] == [
            "Farm/North/M1"
        ]
        assert [p.replace("\\", "/") for p in plan.unmatched_sources] == [
            "Farm/South/M1"
        ]
    ...
    def test_yololabeler_format_finds_the_folder_holding_detect_and_segment(
        self, tmp_path
    ):
        src, img = tmp_path / "preds", tmp_path / "images"
        make_image(img / "Farm/North/M1", "a")
        ...
        assert [p.rel_path.replace("\\", "/") for p in plan.pairs] == [
            "Farm/North/M1"
        ]
...
class TestBatchImportDryRun:
    def test_writes_nothing(self, tree):
        ...
        batch_import(
            plan_pairs(src, img, "bur_detect_json"), "m", 0, "z", dry_run=True
        )
    ...
    def test_reports_accurate_counts(self, tree):
        ...
        assert all(
            r.pair.source_files == 1 and r.pair.images == 1 for r in results
        )
    ...
    def test_requires_class_id(self, tree):
        ...
        with pytest.raises(ValueError, match="class id"):
            batch_import(
                plan_pairs(src, img, "bur_detect_json"),
                "m",
                None,
                "z",
                dry_run=True,
...
class TestBatchImportWrite:
    def test_matches_per_pair_import_predictions(self, tree, tmp_path):
        src, img = tree
        results = batch_import(
            plan_pairs(src, img, "bur_detect_json"), "m", 0, "z"
        )
        assert len(results) == len(MISSIONS)
        for rel in MISSIONS:
            stem = stem_for(rel)
            ...
            assert written.read_text(encoding="utf-8") == (
                solo / "predictions" / "detect" / f"{stem}.txt"
    ...
    def test_manifest_records_the_source_dir_of_that_pair(self, tree):
        src, img = tree
        batch_import(
            plan_pairs(src, img, "bur_detect_json"), "nathan_v15", 0, "zack"
        )
        for rel in MISSIONS:
    ...
    def test_rollup_counts_skipped_and_rejected(self, tmp_path):
        ...
        assert (
            "1 files skipped" in total.summary()
            and "1 lines rejected" in total.summary()
        )
