...
# Centre-match tolerance carried over from the original code, used by the
# migration.
MATCH_TOLERANCE = 0.002
# Fallback cutoff when there is no manifest min_conf; the user set it to the
# conf inference runs at.
...
class QueueItem:
    ...
    def key(self):
        ...
        return (
            self.prediction.id
            if self.kind in ("fp", "tp")
            else self.annotation.id
        )

    @property
    def class_id(self):
        """The class id: the prediction's if present, else the annotation's."""
        return (
            self.prediction.class_id
            if self.prediction
            else self.annotation.class_id
        )


def _split_predictions(predictions):
    """Split a Prediction list into (box list, polygon list), original order
    kept.
    """
...
def match_document(document, predictions, iou_threshold, conf_threshold):
    ...
    pred_boxes = [
        (*p.points[0], *p.points[1], p.class_id, p.confidence) for p in pboxes
    ]
...
def _centre(item):
    """Centre of the item's prediction, or of its annotation when it has no
    prediction.
    """
...
def spatial_order(items):
    """Order items along a nearest-neighbour path so neighbours are visited in
    a row.

    The path starts at the item nearest the image's top-left corner and always
    steps to the closest item not yet visited; ties fall back to the item key
    ...
    while remaining:
        if current is None:
            ...
            key = min(
                remaining,
                key=lambda k: (
                    (centres[k][0] - cx) ** 2 + (centres[k][1] - cy) ** 2,
                    k,
                ),
            )
...
def build_queue(
    ...
    """Flatten matches into QueueItems in spatial order, then filter (spec
    4.4).
    ...
    flagged holds the keys with an open flag, used by the "flagged" status
    filter.
    """
    ...
    for gt_type, gt_idx, p_type, p_idx, iou, _cid, _conf in matches["tp"]:
        items.append(
            QueueItem("tp", pr(p_type, p_idx), gt(gt_type, gt_idx), iou)
        )
    ...
    ordered = spatial_order(
        [item for item in items if keep_type_and_class(item)]
    )
    return [item for item in ordered if keep_status(item)]


def unmatched_prediction_ids(predictions, matches):
    """The ids of the predictions matches left as FPs; empty until matching has
    run.
    """
    if not matches:
        return set()
    pboxes, ppolys = _split_predictions(predictions)
    return {
        (pboxes if p_type == "box" else ppolys)[idx].id
        for p_type, idx, _cid, _conf in matches["fp"]
    }


def shape_statuses(document, predictions, matches, verdicts):
    """Map each queue item's prediction id and annotation id to its review
    status.
...
def flag_markers(document, predictions, matches, open_keys):
    """Map each open-flagged key to the points of the shape that carries its
    mark.
    """
    return {
        key: shape.points
...
def flagged_shapes(document, predictions, matches, open_keys):
    """The ids of the shapes that carry a flag mark: an item's annotation, else
    its prediction.
    """
    return {
        shape.id
        for _, shape in _flagged(document, predictions, matches, open_keys)
    }


def apply_accept(document, item, user):
    """Accept the item; an fp becomes an annotation clamped to the image (spec
    3.3).
    """
    if item.kind == "fp":
        p = item.prediction
        points = [
            (max(0, min(document.width, x)), max(0, min(document.height, y)))
            for x, y in p.points
        ]
        created = new_annotation(
            p.kind,
            points,
            p.class_id,
            user,
...
class ReviewEngine:
    ...
    def load_review_state(self):
        """Read review_stats.json; returns the quarantined path if it was
        corrupt.
        """
    ...
    def save_review_state(self):
        ...
        try:
            ...
            self.on_error(
                f"Could not save review_stats.json: {e.strerror or e}"
            )
    ...
    def conf_threshold(self):
        """The cutoff typed for the current import, else that import's
        min_conf, else the default.

        A typed cutoff is stored with the imported_at of the manifest it was
        typed against, so re-running the import resets the cutoff to the new
        set's own minimum instead of keeping a value chosen for older
        predictions. A cutoff saved before this stamp existed carries none and
        yields to min_conf.
        """
        settings = self.state._review_state.get("settings", {})
        manifest = self._manifest() or {}
        if "conf_threshold" in settings and settings.get(
            "conf_for_import"
        ) == manifest.get("imported_at"):
            return float(settings["conf_threshold"])
        min_conf = manifest.get("min_conf")
        return (
            float(min_conf) if min_conf is not None else DEFAULT_CONF_THRESHOLD
        )
    ...
    def _manifest(self):
        ...
        return read_manifest(
            os.path.join(self.state.image_folder, "predictions")
        )
    ...
    def carry_verdict(self, img_name, item, old_verdict):
        """Carry a prior verdict forward onto item's current key after a
        geometry edit reclassified it (e.g. a confirmed match became a
        standalone miss). Keeps the original reviewer and timestamp; refreshes
        only the fields that describe the item's classification, since the
        edit changed those.
        """
    ...
    def flags(self, img_name):
        """The live flag history for an image: flag key -> list of flag
        entries, oldest first.

        A flag key is a QueueItem.key, or an annotation id for a flag raised on
        a selected annotation with no review queue (a blind image, or one
        without predictions). A resolved entry is kept for the record; only the
        last entry of a key can be open.
        """
        return self._image_entry(img_name).setdefault("flags", {})

    def open_flag(self, img_name, key):
        """The open flag entry for key, or None when it has none or its last
        one is resolved.
        """
    ...
    def open_flag_keys(self, img_name):
        """Keys with an open flag on an image, whether or not their item still
        exists.
        """
        return {
            key
            for key in self.flags(img_name)
            if self.open_flag(img_name, key)
        }
    ...
    def has_open_flags(self, img_name):
        ...
        return any(
            history and not history[-1]["resolved"]
    ...
    def flag_key(self, img_name, item):
        ...
        """
        if item.annotation is not None and self.open_flag(
            img_name, item.annotation.id
        ):
            return item.annotation.id
        return item.key

    def save_flag(self, img_name, key, kind, class_id, comment, user):
        """Open a flag on key with comment, or edit the comment of its open
        flag, and save.

        kind is the item's fp/fn/tp type, or None for an annotation flagged
        with no review queue. Editing keeps the original flagger and records
        the editor.
        """
    ...
    def resolve_flag(self, img_name, key, user, note=None):
        """Mark the open flag on key resolved, keeping it in the history, and
        save.

        note says why when something other than the Resolve flag button
        resolved it, e.g. rejecting or deleting the annotation the flag was
        opened from.
        """
    ...
    def carry_flags(self, img_name, old_key, new_key):
        """Move a key's flag history onto the key a geometry edit reclassified
        it to, and save.
        """
    ...
    def update_img_status(self, img_name):
        """Recompute not_started/started/completed for img_name from every
        queue item, ignoring active filters.
        """
        s = self.state
        verdicts = self.verdicts(img_name)
        if s.document is None or not s.matches:
            ...
            all_items = build_queue(
                s.document, s.predictions, s.matches, verdicts
            )
            total = len(all_items)
    ...
    def migrate_centre_entries(self, img_name, predictions, width, height):
        """Re-key old centre-matched verdict entries by prediction id (spec
        6.3).
        """
        entry = self.state._review_state.get("image", {}).get(img_name)
        ...
        for old in legacy:
            ...
            for p in predictions:
                ...
                if (
                    abs(pcx - cx) / width < MATCH_TOLERANCE
            ...
            verdicts[hit.id] = {
                ...
                "class_id": old.get("class_id"),
                "conf": old.get("conf"),
                "iou": old.get("iou"),
                "by": old.get("reviewed_by", ""),
                "at": "",
            }
        self.save_review_state()
        return dropped
    ...
    def backup_original_labels(self):
        """Copy label files to .original/ before the first label write;
        idempotent.
        """
        s = self.state
