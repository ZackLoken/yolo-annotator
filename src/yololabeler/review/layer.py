"""Draw the prediction layer and the focused pair on the annotate canvas (spec
4.5).
...
# Green/orange/red for accepted/not reviewed/rejected; the user swapped yellow
# for orange as easier on the eyes. The hex values are provisional.
STATUS_COLORS = {
    "accepted": "#00FF00",
    "not_reviewed": "#FF8C00",
    "rejected": "#FF0000",
}
# The mark a flagged shape's label ends with; the user chose it. The colour is
# the legend swatch's; on the canvas the mark takes its label's colour.
FLAG_COLOR = "#FFFFFF"
FLAG_MARK = "?"


def label_text(class_id, class_names, flagged, confidence=None):
    """The label a shape is drawn with: "id: name", the confidence when given,
    the flag mark when flagged.
    """
...
def status_colors_active(state):
    """The id-to-status map to colour shapes by, or None when they keep class
    colours.
...
def status_color(statuses, shape_id):
    """The status colour for a shape id, treating an id with no entry as not
    reviewed.
    """
    return STATUS_COLORS.get(
        statuses.get(shape_id), STATUS_COLORS["not_reviewed"]
    )
...
class LayerStyle:
    ...
    pred_color is only the fallback for when the caller supplies no
    class-colour lookup. Line widths are not here: the caller passes its own
    scale-dependent width so predictions and annotations stay equally thick at
    every zoom.
    """
    ...
    # Tk on Windows collapses a numeric dash list to a dotted look; the string
    # form stays a dash.
    dash: str = "_"
...
def draw_prediction_layer(
    ...
    """Draw predictions, the focused item and the badge; the focus and every FP
    get a label.
    ...
    focused_pred_id = (
        focused.prediction.id if focused and focused.prediction else None
    )
    ...
    if show_pred:
        # An FP has no annotation to carry a label, so it gets its own; a TP's
        # sits on its GT.
    ...
    if labelled is not None and labelled.id != selected_id:
        ...
        text = label_text(
            focused.class_id,
            class_names,
            labelled.id in state.flagged_shapes,
            confidence,
        )
        ...
        place_label(
            ...
            ly - 2,
            text,
            label_color_of(focused.class_id),
            anchor="sw",
            font=font,
        )
    ...
    if show_gt or show_pred:
        ...
        if focused.key in state.flag_markers or (
            focused.annotation is not None
            and focused.annotation.id in state.flag_markers
        ):
            text += "  flagged"
