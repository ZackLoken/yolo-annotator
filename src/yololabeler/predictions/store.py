"""Load canonical prediction files with stable sha1+line ids, GUI-free (spec
3.2).
"""
...
def load_predictions(pred_detect_dir, pred_segment_dir, stem, width, height):
    """Read an image stem's detect and segment prediction files, returning
    ...
    for kind, folder in (
        ...
        for row in parsed.rows:
            predictions.append(
                Prediction(
                    id=prediction_id(digest, row.line_index),
                    kind=kind,
...
def read_manifest(predictions_dir):
    """Return the parsed manifest.json under predictions_dir, or None if
    absent.

    A manifest that fails to parse is quarantined beside its folder, like every
    other state file, and read as absent.
    """
    data, _moved = read_json_or_quarantine(
        os.path.join(str(predictions_dir), MANIFEST_NAME)
    )
    return data
...
def write_manifest(predictions_dir, manifest):
    ...
    write_json_atomic(
        os.path.join(str(predictions_dir), MANIFEST_NAME), manifest
    )
