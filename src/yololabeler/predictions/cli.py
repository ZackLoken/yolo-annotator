"""`yololabeler-import`: batch-import a prediction tree into an image tree, headless.
...
def _display(rel_path):
    """Relative path in forward slashes, as '(root)' for the roots themselves."""
    return "(root)" if rel_path == "." else rel_path.replace(os.sep, "/")


def _is_within(child, parent):
    """True when child is parent or lives underneath it, case-insensitively on Windows."""
...
def _print_plan(plan):
    """Print the pairing table, both unmatched lists and any cross-folder stem collisions."""
    ...
    for pair in plan.pairs:
        print(f"  {_display(pair.rel_path):<{width}}  "
    ...
    for title, rel_paths in (
        ...
        if not rel_paths:
            print("  (none)")
    print()
    print(f"Source stems found under more than one source folder "
...
def _build_parser():
    """Build the argument parser; dry run is the default and --write is the real run."""
    parser = argparse.ArgumentParser(
        prog="yololabeler-import",
        description="Batch-import a tree of prediction folders into the matching tree "
                    "of image folders, pairing strictly by relative path. Previews the "
                    "pairing without writing unless --write is given.")
    parser.add_argument("source_root", help="root of the tree of source prediction folders")
    parser.add_argument("images_root", help="root of the tree of image folders")
    parser.add_argument("--format", dest="fmt", required=True, choices=FORMATS,
                        help="source prediction format")
    parser.add_argument("--model", required=True, help="model name recorded in each manifest")
    parser.add_argument("--class-id", type=int, default=None,
                        help="class id for bur_detect_json, which carries none; "
                             "ignored by the other formats")
    parser.add_argument("--user", default="", help="name recorded as imported_by")
    ...
    mode.add_argument("--write", dest="write", action="store_true",
                      help="actually convert and write predictions/ under every image folder")
    return parser


def main(argv=None):
    """Plan the pairing, print it, and import when --write was given. Returns an exit code."""
    parser = _build_parser()
    args = parser.parse_args(argv)
    source_root = os.path.realpath(args.source_root)
    images_root = os.path.realpath(args.images_root)
    for root, label in ((source_root, "source root"), (images_root, "images root")):
        if not os.path.isdir(root):
            parser.error(f"{label} is not a directory: {root}")
    if source_root == images_root:
        parser.error("source root and images root are the same folder; a batch job may "
                     "not walk the tree it writes into")
    if _is_within(source_root, images_root) or _is_within(images_root, source_root):
        parser.error("source root and images root are nested; a batch job may not walk "
                     "the tree it writes into")
    if args.fmt == "bur_detect_json" and args.class_id is None:
        parser.error("bur_detect_json files carry no class; --class-id is required")
    ...
    for pair_result in results:
        print(f"{_display(pair_result.pair.rel_path)}: {pair_result.summary()}")
    print()
