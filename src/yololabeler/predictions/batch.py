"""Pair a source prediction tree with an image tree and import every folder.
GUI-free.
"""
...
class Pair:
    """One source folder matched to one image folder by their shared relative
    path.
    """
...
@dataclass
class BatchPlan:
    """Matched pairs, the folders that paired with nothing, and colliding
    source stems.
    """
...
@dataclass
class PairResult:
    """One pair's import outcome; result is None on a dry run, which writes
    nothing.
    """
    ...
    def summary(self):
        ...
        if self.result is None:
            return (
                f"{self.pair.source_files} source files, "
...
class BatchTotals:
    ...
    def summary(self):
        """One-line summary of the whole batch, for the console."""
        noun = "folder" if self.pairs == 1 else "folders"
        parts = [
            f"Imported predictions for {self.files_written} images "
            f"across {self.pairs} {noun}"
        ]
        if self.files_skipped:
            parts.append(
                f"{self.files_skipped} files skipped (no matching image)"
            )
...
def _image_dirs(images_root):
    """rel_path -> image count, for every directory under images_root holding
    images.
    """
    found: dict[str, int] = {}
...
def _source_dirs(source_root, fmt):
    """rel_path -> [stem], for every directory under source_root holding source
    files.
    """
...
def _collisions(source_stems):
    """stem -> sorted rel_paths, for every stem found under more than one
    source folder.
    """
...
def plan_pairs(source_root, images_root, fmt):
    """Pair source folders to image folders strictly by relative path, without
    writing.

    A source folder matches an image folder only when their paths relative to
    their own roots are identical; a leaf folder name matching somewhere else
    in the tree is not a match, and source files are never indexed by stem
    across folders. Folders that pair with nothing are reported, not resolved.
    """
...
def batch_import(plan, model_name, class_id, user, dry_run=False):
    """Run import_predictions for every matched pair; write nothing when
    dry_run.

    The format comes from the plan, so the counts previewed by a dry run and
    the files written by the real run can never be read with two different
    formats.
    """
...
def totals(results):
    """Sum every pair's ImportResult into one BatchTotals; dry-run pairs add
    nothing.
    """
