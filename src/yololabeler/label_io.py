...
class ParsedRow:
    """One accepted line of a label or prediction file, in pixel coordinates."""
    class_id: int
    points: Tuple[Point, ...]
...
@dataclass(frozen=True)
class ParsedFile:
    """Accepted rows plus the 1-based numbers of lines that could not be parsed."""
    rows: List[ParsedRow]
    rejected: List[int]
...
def parse_label_file(path, kind, img_w, img_h, with_conf=False):
    """Parse a detect or segment file into pixel rows, reporting rejected lines.
...
def _parse_line(line, index, kind, img_w, img_h, with_conf):
    ...
    if kind == "box":
        if len(vals) != 4:
            return None
        xc, yc, w, h = vals[0] * img_w, vals[1] * img_h, vals[2] * img_w, vals[3] * img_h
        points = ((xc - w / 2, yc - h / 2), (xc + w / 2, yc + h / 2))
    else:
        if len(vals) < 6 or len(vals) % 2:
            return None
        points = tuple((vals[i] * img_w, vals[i + 1] * img_h)
                       for i in range(0, len(vals), 2))
    return ParsedRow(class_id, points, confidence, index, line)


def write_json_atomic(path, data):
    """Write JSON through a temp file and os.replace so a crash keeps the old file."""
    path = str(path)
...
def write_detect_labels(path, boxes, img_w, img_h):
    ...
    """
    lines = [format_detect_line(x1, y1, x2, y2, cls, img_w, img_h) + "\n"
...
def write_segment_labels(path, polygons, img_w, img_h):
    ...
    img_w, img_h : int
        Image dimensions for normalisation.
    """
    lines = [format_segment_line(points, cls, img_w, img_h) + "\n"
